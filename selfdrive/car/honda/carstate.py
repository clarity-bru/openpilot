...
class CarState(CarStateBase):
  ...
  def update(self, cp): #Clarity: cp_cam is the CAN parser for the Factory Camera CAN. Since we've disconnected the factory camera, this is not needed. -wirelessnet2
    ...
    self.tripDistance = cp.vl["ENGINE_DATA"]['ODOMETER']
    ...
    if self.CP.carFingerprint in HONDA_BOSCH:
      ...
      self.stock_fcw = bool(0) #Clarity: Since we don't have the Factory ADAS Camera connected, we will never need these. If we leave it in, it calls on cp_cam and causes errors. -wirelessnet2
      ...
      self.stock_brake = 0 #Clarity: Since we don't have the Factory ADAS Camera connected, we will never need these. If we leave it in, it calls on cp_cam and causes errors. -wirelesset2
